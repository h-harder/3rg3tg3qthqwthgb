...
find_source_dir() {
  local here
  here="$(cd "$(dirname "${BASH_SOURCE[0]}")" >/dev/null 2>&1 && pwd || pwd)"
  if [ -f "$here/package.json" ] && [ -f "$here/server.js" ]; then
    printf '%s\n' "$here"
    return 0
  fi
...
for (const [key, value] of Object.entries(values)) {
  ...
  if (re.test(text) && !text.match(re)[0].includes('CHANGE_ME')) continue;
...
<dict>
  ...
  <key>StandardOutPath</key><string>$INSTALL_DIR/data/server.log</string>
  <key>StandardErrorPath</key><string>$INSTALL_DIR/data/server.log</string>
</dict>
</plist>
EOF2
...
#!/usr/bin/env bash
cd "$INSTALL_DIR"
exec "$(command -v node)" bin/chatctl.js "\$@"
EOF2
...
start_service() {
  cd "$INSTALL_DIR"
  rm -f data/server.paused
  if [ "$(uname -s)" = "Linux" ]; then
    systemctl --user start simple-chat.service
  elif [ "$(uname -s)" = "Darwin" ]; then
    launchctl bootout "gui/$(id -u)/com.simplechat.server" >/dev/null 2>&1 || true
    launchctl bootstrap "gui/$(id -u)" "$HOME/Library/LaunchAgents/com.simplechat.server.plist" >/dev/null 2>&1 || true
    launchctl kickstart -k "gui/$(id -u)/com.simplechat.server"
  fi
}
